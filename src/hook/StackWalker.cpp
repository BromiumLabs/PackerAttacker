/* this is an open source class from Code Project. It was not made by or for the creators of The Packer Attacker*/
/**********************************************************************
 * 
 * StackWalker.cpp
 *
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    if (s.AddrPC.Offset != 0)
    {
      ...
      if (this->m_sw->pSGSFA(this->m_hProcess, s.AddrPC.Offset, &(csEntry.offsetFromSmybol), pSym) != FALSE)
      {
		...
		sprintf(csEntry.name, "%s + 0x0%x", pSym->Name, csEntry.offsetFromSmybol);

        // UnDecorateSymbolName()
		char tmpund[1024];
		char tmpundFull[1024];
        this->m_sw->pUDSN( pSym->Name, tmpund, STACKWALK_MAX_NAMELEN, UNDNAME_NAME_ONLY );
        this->m_sw->pUDSN( pSym->Name, tmpundFull, STACKWALK_MAX_NAMELEN, UNDNAME_COMPLETE );

		sprintf(csEntry.undName, "%s + 0x0%x", tmpund, csEntry.offsetFromSmybol);
		sprintf(csEntry.undFullName, "%s + 0x0%x", tmpundFull, csEntry.offsetFromSmybol);
      }
      else
      {
